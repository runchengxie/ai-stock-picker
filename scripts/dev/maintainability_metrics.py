...
def _c901_inline_ignore_count(text: str) -> int:
    ...
    for comment in comments:
        ...
        if (
            directive.casefold() == "noqa"
            and comment.start[0] in function_definition_lines
        ):
            count += 1
            continue
        ...
        if prefix.strip().casefold() != "noqa" or not separator:
            continue
        normalized_codes = {
            code.strip().upper() for code in codes.replace(",", " ").split()
        }
...
def collect_metrics(
    ...
    largest_files = sorted(file_metrics, key=lambda item: item.lines, reverse=True)[
        :limit
    ]
    ...
    return Metrics(
        ...
        tests_over_1000=sum(
            ...
            if item.lines > 1000 and item.path.startswith("tests/")
        ),
...
def format_markdown(metrics: Metrics) -> str:
    ...
    for item in metrics.largest_functions:
        lines.append(
            f"| {item.lines} | `{item.name}` | `{item.path}:{item.start_line}` |"
        )
    return "\n".join(lines)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--markdown", action="store_true", help="Markdown table output."
    )
    parser.add_argument(
        "--limit",
        type=int,
