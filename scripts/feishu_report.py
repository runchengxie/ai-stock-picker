...
_MARKET_INTEL_ROOT = Path(
    os.environ.get(
        "MARKET_INTEL_ROOT",
        str(Path.home() / "code/market-intel"),
    )
)
...
def build_report(date_str: str, top_n: int = 10) -> dict[str, Any]:
    ...
    if not picks_path.exists():
        return _error_card(
            date_str, "今日 AI 选股尚未生成。请先运行 deepseek_pick.py。"
        )
    ...
    topic_lines = [
        f"{t}（{n}只）" for t, n in sorted(topics.items(), key=lambda x: -x[1])
    ]
...
def deliver_report(card: dict, dry_run: bool = True) -> bool:
    ...
    post_feishu = (
        _MARKET_INTEL_ROOT / "src" / "daily_messenger" / "tools" / "post_feishu.py"
    )
...
def _send_status(
    dry_run: bool, ok: bool, elapsed: float, date_str: str, top_n: int
) -> None:
...
# Top-N comparison
# ---------------------------------------------------------------------------
def compare_top_n(date_str: str, top_ns: list[int] = [5, 10, 15]) -> None:
    """Run backtest for multiple top-n values and compare."""
    print(f"Top-N comparison for {date_str}:")
    print(
        f"{'top_n':>8s}  {'Return':>10s}  {'Sharpe':>8s}  {'MaxDD':>8s}  {'HitRate':>8s}"
    )
    print("-" * 55)
...
def main() -> None:
    ...
    parser.add_argument(
        "--top-n", type=int, default=10, help="Stocks to show (default: 10)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
