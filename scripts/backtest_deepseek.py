...
_HOTSECTOR_ROOT = Path(
    os.environ.get(
        "HOTSECTOR_ROOT",
        str(Path.home() / "code/market-intel/hot-sector-screener"),
    )
)
OUTPUTS_DIR = _HOTSECTOR_ROOT / "outputs"
...
def load_trade_dates(start: str, end: str) -> list[str]:
    ...
    ths_dir = (
        DATA_PLATFORM_ROOT
        / "assets/tushare/a_share/ths_hot/a_share_all_ths_hot_latest/data"
    )
    if not ths_dir.exists():
        print(
            f"WARNING: {ths_dir} not found, returning all dates in range",
            file=sys.stderr,
        )
...
def run_phase_picks(dates: list[str], top_n: int, model: str | None) -> list[Path]:
    ...
    for i, date in enumerate(dates):
        ...
        print(
            f"  [{i + 1}/{len(dates)}] {date} — calling DeepSeek ({len(df)} candidates)..."
        )
        try:
...
def run_phase_backtest(
    ...
    for date, pf in zip(dates, pick_files, strict=False):
        if not pf.exists():
            continue
        data = json.loads(pf.read_text())
        picks_index[date] = [
            {"symbol": p["ts_code"], "conf": p["confidence_score"]}
            for p in data["picks"]
        ]
        for p in data["picks"]:
    ...
    for date_str in sorted(picks_index.keys()):
        ...
        for p in picks:
            pos_rows.append(
                {"rebalance_date": sig_date, "symbol": p["symbol"], "weight": w}
            )
    ...
    for i, sig_date in enumerate(sorted_dates[:-1]):
        entry_date = (
            all_trade_dates[i + 1] if i + 1 < len(all_trade_dates) else sig_date
        )
        exit_date = (
            all_trade_dates[i + 2] if i + 2 < len(all_trade_dates) else entry_date
        )
        period_rows.append(
            {
    ...
    print(
        f"Baseline: {len(base_positions)} positions, {base_positions.rebalance_date.nunique()} dates"
    )
    ...
    for key, label in [
        ...
        sv = s.get(key)
        bv = bs.get(key)
        if isinstance(sv, (int, float, np.floating)) and isinstance(
            ...
            if key in (
                ...
                print(
                    f"{label:<25s} {float(sv) * 100:>13.2f}% {float(bv) * 100:>13.2f}%"
                )
            else:
                print(f"{label:<25s} {float(sv):>14.3f} {float(bv):>14.3f}")
        else:
            print(f"{label:<25s} {str(sv):>14s} {str(bs):>14s}")
    print("=" * 65)
    ...
    summary = {
        ...
        "baseline": {
            k: float(v) if isinstance(v, (np.floating,)) else v for k, v in bs.items()
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Hot-sector + DeepSeek picks → backtest pipeline"
    )
    parser.add_argument(
        "--phase",
    ...
    if not trade_dates:
        print(
            f"ERROR: no trade dates found in [{start_date}, {end_date}]",
            file=sys.stderr,
        )
        sys.exit(1)
