...
# Reads from market-intel hot-sector-screener outputs (the data pipeline)
...
def build_stock_pick_prompt(
    ...
    lines = [
        ...
        "### 候选股票明细",
        "",
        "| ts_code | 名称 | 得分 | 关联主题 | 关联概念 | 日内确认 | 趋势 | 量能 | 风险 | 流动性 | 置信度 |",
        "|---------|------|------|----------|----------|----------|------|------|------|--------|--------|",
    ]
    ...
    lines.extend(
        [
            ...
            "",
            "### 一级：量价共振（权重最高）",
            "- **量能分 ≈ 1.0** 且 **日内确认分 > 0.6**：说明有真金白银在买，且盘中走势验证了题材逻辑。这种量价配合是短线最可靠的信号。",
            ...
            "",
            "### 三级：风险过滤",
            "- **风险分 < 0.3** 的股票大概率是连板投机票或问题股，除非量能+趋势+日内确认三项都接近满分，否则回避",
            ...
            "",
            "示例:",
            '[{"ts_code": "000001.SZ", "name": "平安银行", "confidence_score": 8, "reasoning": "...", "primary_topic": "金融科技", "risk_note": ""}]',
        ]
    )
...
def main() -> None:
    ...
    if not run_date:
        print(
            "ERROR: no --date given and no outputs/ directories found.", file=sys.stderr
        )
        sys.exit(1)
    ...
    # Call DeepSeek
    print(
        f"Calling DeepSeek (model={args.model or 'deepseek-chat'}, top_n={args.top_n})..."
    )
    try:
