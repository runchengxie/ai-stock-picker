...
A 股正式选择只使用 `DEEPSEEK_API_KEY`。`bounded_ranking_v2` 研究 shadow 可以使用
`OPENAI_API_KEY`，该例外不会生成正式发布选择。
...
负责冻结的 bounded v2 单日研究执行：三次 repetition、至少两次有效、每单元
complete/tombstone、固定 Top7、边界票数/Borda/中位序/symbol 共识，以及无网络
watchdog。目录按 `campaign/provider--model/date/repetition` 分区，旧 evidence 和 v1
artifact 不会被改写。
